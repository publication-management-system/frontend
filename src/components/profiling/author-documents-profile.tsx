...
import type { DocumentPayload } from "../../data/document-payload.ts";
...
export const AuthorDocumentsProfile = (props: AuthorDocumentsProfileProps): React.JSX.Element => {
    ...
    const fetchScrapedProfile = async (pageNumber: number) => {
        ...
        await authenticatedClient
            .get(
                `/api/scraped-entities/${props.sessionId}?source=${props.dataSource.toUpperCase()}&entityType=DOCUMENT&pageNumber=${pageNumber}&pageSize=10`,
            )
            .then((response) => {
                setDocuments({
                    loading: false,
    ...
    return documents.loading ? (
        ...
        <div className={"document-accordion-item"}>
            ...
            <Pageable
                ...
                onPageChange={(pageNumber) => fetchScrapedProfile(pageNumber)}
            />
        </div>
    );
};
