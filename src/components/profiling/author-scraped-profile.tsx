...
import type { DataSource, PagedScrapedEntity } from "../../data/scraped-entity.ts";
...
export const AuthorScrapedProfile = (props: AuthorScrapedProfileProps): React.JSX.Element => {
    ...
    useEffect(() => {
        const fetchScrapedProfile = async () => {
            ...
            await authenticatedClient
                .get(
                    `/api/scraped-entities/${props.sessionId}?source=${props.dataSource.toUpperCase()}&entityType=AUTHOR&pageNumber=0&pageSize=1`,
                )
                .then((response) => {
                    setAuthorProfile({
    ...
    const authorProfilePayload = JSON.parse(
        authorProfile.entities.length > 0 ? authorProfile.entities[0].payload : "{}",
    );
    return authorProfile.loading ? (
        <LoadingAuthorScrapedProfile />
    ) : (
        <div className="author-scraped-profile">
            <img
                src={authorProfilePayload?.imageUrl ?? "/missing_image.png"}
                alt={"Profile image"}
            />
            <div>
...
export const LoadingAuthorScrapedProfile = (): React.JSX.Element => {
    return <div className={"loading_bg_skeleton"} style={{ width: "100%", height: 400 }}></div>;
};
