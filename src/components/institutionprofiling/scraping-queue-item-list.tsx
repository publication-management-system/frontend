...
import type { ScrapingQueueItem } from "../../data/scraping.ts";
import { Button } from "../button/button.tsx";

import "./scraping-queue-item-list.css";

export default function ScrapingQueueItemList({
    onRunning,
}: {
    onRunning: () => void;
}): React.JSX.Element {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            await authenticatedClient
                .get(`/api/scraping/next-in-queue?userId=${userId}`)
                .then((response) => {
                                ...
                                {items.map((item: ScrapingQueueItem) => (
                                    <div className={"scraping-item-list-item"} key={item.id}>
                                        <div className={"scraping-item-list-item-icon"}>
                                            {item.type.includes("AUTHOR") && (
                                                <HiOutlineAcademicCap />
                                            )}
                                            {item.type.includes("DOCUMENT") && (
                                                <HiOutlineDocumentSearch />
                                            )}
                                            {item.type.includes("CITATION") && (
                                                <HiOutlineExternalLink />
                                            )}
                                        </div>

                                        <p className={"scraping-item-list-item-text"}>
                                            {item.type.toUpperCase().replace("_", " ")}
                                        </p>
                                        {item.scrapingLink && (
                                            <p className={"scraping-item-list-item-text"}>
                                                {`${item.scrapingLink.slice(0, 30)}...`}
                                            </p>
                                        )}
                                        {item.payload && (
                                            <p className={"scraping-item-list-item-text"}>
                                                {`${item.payload.slice(0, 30)}...`}
                                            </p>
                                        )}
                                    </div>
                                ))}
